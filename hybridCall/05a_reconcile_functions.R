...
ChainToDFlist <- function(chain_file, n_chrs = 16) {
  ...
  # read in chain file
  chain_table <- readLines(chain_file) 
  # remove spurious "\" characters
  chain_table <- gsub(pattern = "\"", replacement = "", x = chain_table)
  # remove new lines at end of each section
  chain_table <- chain_table[!chain_table == ""]
  # index section headers
  chr_idx <- grep("chain", chain_table)
  ...
  # dataframe of section blocks
  value_idx <- data.frame(from = chr_idx + 1, 
                          to = c(chr_idx[2:length(chr_idx)] - 1, end_idx))
  # get section headers
  chr_headers <- chain_table[chr_idx]
  # turn headers into dataframe
  chr_info <- str_split(chr_headers, " ", simplify = T) %>% as.data.frame(stringsAsFactors = F)
  col_names <- c("CHROM", "length", "strand", "start", "end")
  ...
  chrom_names <- str_pad(1:nrow(chr_info), width = 2, side = "left", pad = "0")
  ...
  for (ch in 1:nrow(chr_info)) {
    ...
    if(value_idx$from[ch] == value_idx$to[ch]) {
      sub_df <- data.frame(block = chr_info$Ref_length[ch], Ref_diff = 0, Qry_diff = 0, 
                           cs_block = chr_info$Ref_length[ch], cs_Ref_diff = 0, cs_Qry_diff = 0, 
                           Ref_POS = chr_info$Ref_length[ch], Qry_POS = chr_info$Ref_length[ch],
                           Ref_POSi = Ref_chrom_starts[ch] + chr_info$Ref_length[ch], 
                           Qry_POSi = Ref_chrom_starts[ch] + chr_info$Ref_length[ch],
                           CHROM = chrom_names[ch])
    } else {
      ...
      # cs_block is 0 indexed, so add 1 for position
      sub_df$Ref_POS <- sub_df$cs_block + cumsum(sub_df$Ref_diff) + 1 
      sub_df$Qry_POS <- sub_df$cs_block + cumsum(sub_df$Qry_diff) + 1
      sub_df[1, c("Ref_POS", "Qry_POS")] <- sub_df$cs_block[1] + 1
      # chrom_starts is 1 indexed, so -1 to add intervening chroms
      sub_df$Ref_POSi <- sub_df$Ref_POS + Ref_chrom_starts[ch] - 1 
...
HC_multiVcfParse_allVar <- function(vcf_file., max_chrs = 16, verbose = F) {
  ...
  if(max(vcf_in$chrom_n) > (max_chrs)) {
...
MarkRepeats <- function(vcf_in, repeats_in) {
  ...
  #   }
  # }
  repeats_seq <- mapply(FUN = function(x, y) seq(x, y, 1), 
                        x = repeats_bed$start_POSi, 
                        y = repeats_bed$end_POSi) %>% 
                  unlist()
  rpt <- vcf_in$POSi %in% repeats_seq
  return(rpt)
}
