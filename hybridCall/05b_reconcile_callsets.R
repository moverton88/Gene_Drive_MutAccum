...
# P1_file <- paste0(proj_dir, "variants/final/BY/all_BY.filter.vcf")
# P2_file <- paste0(proj_dir, "variants/final/RM/all_RM.filter.vcf")
# P1_chainFile <- paste0(proj_dir, "/references/final/BY/BYxS288C.chain")
# P2_chainFile <- paste0(proj_dir, "/references/final/RM/RMxS288C.chain")
# repeats_bed <- paste0(proj_dir, "/references/final/S288C_repeats.bed")
# output_table <- paste0(proj_dir, "/variants/final/Dual/all_BYxRM_reconciled_SNPs.RData")
...
if(P1 != "S288C") {
    cat(paste0("P1 is not strain S288C, attempting to open P1 chain file\n", P1_chainFile))
...
cat(paste0("\nP2 is not strain S288C, attempting to open P2 chain file\n", P2_chainFile))
...
all_var_P1_lo <- all_var_P1 %>% filter(ID == "F_A00") %>%
  ConstructLiftover(.,  chain_df = P1toRef_chainDf, 
                    chrom_from = chrom_bounds_P1,
                    chrom_to = chrom_bounds_Ref)
...
all_var_P2 <- all_var_P2 %>%
    ConstructLiftover(.,  chain_df = P2toRef_chainDf, 
                      ...
                      chrom_to = chrom_bounds_Ref)
# all_var_P2_lo <- all_var_P2 %>% filter(ID == "wt_00") %>%
#   ConstructLiftover(.,  chain_df = P1toRef_chainDf, 
#                     chrom_from = chrom_bounds_P2,
#                     chrom_to = chrom_bounds_Ref)
...
if(merge_QC == 1) {
    cat("\nMerged callset length close to sum of individual callset legnths and 
           may be malformed\n")
}
...
if(min_QUAL > 0) {
    merge_stats <- SNPs_merge %>%
        filter(QUAL_P1call >= min_QUAL | QUAL_P2call >= min_QUAL) %>%
        summarize(n_P1calls = sum(!is.na(GT_P1call)),
                  n_P2calls = sum(!is.na(GT_P2call)),
                  sum_n_calls = (n_P1calls + n_P2calls),
                  mean_n_calls = (sum_n_calls / 2),
                  n_intersect = sum(!is.na(GT_P1call) & !is.na(GT_P2call)),
                  f_P1intersect = n_intersect / n_P1calls,
                  f_P2intersect = n_intersect / n_P2calls,
                  n_exclusive = sum(is.na(GT_P1call) | is.na(GT_P2call)))
} else {
     merge_stats <- SNPs_merge %>%
        summarize(n_P1calls = sum(!is.na(GT_P1call)),
                  n_P2calls = sum(!is.na(GT_P2call)),
...
cat("After removing repeats, merge stats are\n")
...
reconcile_stats <- data.frame(n_reconciled = nrow(SNPs_merge_finalGT)) %>%
                        mutate(p_P1call_rec = n_reconciled/merge_stats$n_P1calls,
                               p_P2call_rec = n_reconciled/merge_stats$n_P2calls)
print(reconcile_stats)
...
output_dir <- gsub(basename(output_table), "", output_table)
merge_path <- paste0((output_dir), "merge_stats_list.tsv")
reconcile_path <- paste0((output_dir), "reconcile_stats_list.tsv")
write.table(merge_stats, file = merge_path, sep = "\t",
            col.names = T, row.names = F)
write.table(reconcile_stats, file = reconcile_path, sep = "\t",
            col.names = T, row.names = F)
}
