...
# Depths under 8 produce BB alphas that result in thresholds
# < 1 and > Sum_DP
# model_min_DP <- 6
...
train_ID <- as.character(input_args[2])
fractionTrainIDs <- as.numeric(input_args[3])
fractionNonTrainIDs <- as.numeric(input_args[4])
input_params <- as.character(input_args[5])
...
if(!any(grepl("Sum_DP", colnames(SNPs_merge_finalGT)))) {
...
valid_DP_values <- varAlt_sumDP %>% 
                    filter(n >= min_n, Sum_DP_mean <= model_max_DP) %>% 
                    pull(Sum_DP_mean)
...
if(length(valid_DP_values) > 10) {
    var_lm_fix_Binom <- varAlt_sumDP %>% 
                            ...
                            lm(var_Alt_resid ~ 0 + I(Sum_DP_mean^2), data = .)
